...
#
# Purpose: Run random forest missing data model for lung function
#
...
comorbid <- read_csv("../data/alive_comorbid.csv") %>% 
  filter(year(visdate) %in% 2007:2019) %>% 
  select(id, visdate, lungdx6m, lungtx6m, evrlungdx, fev_fvc)
...
wide3 <- wide2 %>% 
  mutate(enroll_period=factor(enroll_period), 
         across(c(beduchs, m0f1, black,
                  vars_select(names(wide), starts_with("lungdx6m")), 
                  vars_select(names(wide), starts_with("lungtx6m")), 
                  vars_select(names(wide), starts_with("evrlungdx")),
...
fev <- wide.impute2 %>% 
  ...
  pivot_longer(cols=starts_with("fev"), names_to="n_vis", names_prefix="fev_fvc_", values_to="fev_fvc")
trt <- wide.impute2 %>% 
  select(id, vars_select(names(wide), starts_with("lungtx6m"))) %>% 
  pivot_longer(cols=starts_with("lungtx6m"), names_to="n_vis", names_prefix="lungtx6m_", values_to="lungtx6m")

# Keep imputed values from observed visits
lung <- dat %>% 
  select(id, visdate, n_vis) %>% 
  left_join(fev, by=c("id", "n_vis")) %>% 

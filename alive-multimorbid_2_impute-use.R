...
dat <- select(socdem, -c(m0f1, black)) %>% 
...
long.impute <- wide.impute2 %>% 
  pivot_longer(!c(id, enroll_period, beduchs, m0f1, black), names_to = c(".value", "n_vis"),
               names_pattern = "(.*)_(.*)")
# Keep imputed values from observed visits
use <- dat %>% 
  select(id, visdate, n_vis) %>% 
  left_join(long.impute, by=c("id", "n_vis"))


# Export data -------------------------------------------------------------

write_csv(use, file="../data/alive_use_impute.csv")

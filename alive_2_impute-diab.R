...
wide2 <- wide2 %>% 
  mutate(enroll_period=factor(enroll_period), 
         across(c(beduchs, bempcurr, beverwk, bevinj, bhome1yr, bnevmar,
                  ...
                  vars_select(names(wide), starts_with("work")), vars_select(names(wide), starts_with("diabdx6m")), 
                  vars_select(names(wide), starts_with("diabtx6m")), vars_select(names(wide), starts_with("evrdiab")),
...
write_csv(diab, file="./alive_diab_impute.csv")

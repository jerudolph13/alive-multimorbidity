...
comorbid <- read_csv("../data/alive_mental.csv") %>% 
  mutate(yr = year(visdate)) %>% 
  select(-c(visdate, enroll_period, visit, manddx6m, mandtx6m, emedmand, schidx6m, schitx6m, emedschi,
            ehspmntl, anxdx6m, anxtx6m, emedanx))
...
comorbid_annual <- make_annual(comorbid, sym("cesd23"), sym("depdx6m"), sym("deptx6m"), 
                               sym("emeddep")) %>% 
  mutate(across(c(cesd23, depdx6m, deptx6m, emeddep), ~ ceiling(.x)))
...
wide2 <- wide2 %>% 
  mutate(enroll_period=factor(enroll_period), 
         across(c(beduchs, bempcurr, beverwk, bevinj, bhome1yr, bnevmar,
                  ...
                  vars_select(names(wide), starts_with("work")), vars_select(names(wide), starts_with("cesd23")),
                  vars_select(names(wide), starts_with("depdx6m")), vars_select(names(wide), starts_with("deptx6m")), 
...
cesd <- wide.impute2 %>% 
  select(id, vars_select(names(wide), starts_with("cesd23"))) %>% 
  pivot_longer(cols=starts_with("cesd23"), names_to="yr", names_prefix="cesd23_", values_to="cesd23")
trt <- wide.impute2 %>% 
  select(id, vars_select(names(wide), starts_with("deptx6m"))) %>% 
  pivot_longer(cols=starts_with("deptx6m"), names_to="yr", names_prefix="deptx6m_", values_to="deptx6m")

dep <- left_join(cesd, trt, by=c("id", "yr"))

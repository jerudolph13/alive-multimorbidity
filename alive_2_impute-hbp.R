...
comorbid_annual <- make_annual(comorbid, sym("bpdias"), sym("bpsys"), sym("hbpdx6m"), sym("hbptx6m"), sym("evrhbp")) %>% 
  mutate(across(c(hbpdx6m, hbptx6m), ~ ceiling(.x)),
         bpdias=log(bpdias),
         bpsys=log(bpsys))
...
wide2 <- wide2 %>% 
  mutate(enroll_period=factor(enroll_period), 
         across(c(beduchs, bempcurr, beverwk, bevinj, bhome1yr, bnevmar,
                  ...
                  vars_select(names(wide), starts_with("work")), vars_select(names(wide), starts_with("hbpdx6m")), 
                  vars_select(names(wide), starts_with("hbptx6m")), vars_select(names(wide), starts_with("evrhbp")),
...
sysbp <- wide.impute2 %>% 
  ...
  pivot_longer(cols=starts_with("bpsys"), names_to="yr", names_prefix="bpsys_", values_to="bpsys") %>% 
  mutate(bpsys=exp(bpsys))
diasbp <- wide.impute2 %>% 
  select(id, vars_select(names(wide), starts_with("bpdias"))) %>% 
  pivot_longer(cols=starts_with("bpdias"), names_to="yr", names_prefix="bpdias_", values_to="bpdias") %>% 
  mutate(bpdias=exp(bpdias))
trt <- wide.impute2 %>% 
  select(id, vars_select(names(wide), starts_with("hbptx6m"))) %>% 
  pivot_longer(cols=starts_with("hbptx6m"), names_to="yr", names_prefix="hbptx6m_", values_to="hbptx6m")

hbp <- left_join(sysbp, left_join(diasbp, trt, by=c("id", "yr")), by=c("id", "yr"))

# Export data -------------------------------------------------------------

write_csv(hbp, file="./alive_hbp_impute.csv")

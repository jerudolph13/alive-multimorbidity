...
liver <- wide.impute2 %>% 
  ...
  pivot_longer(cols=starts_with("fbscan"), names_to="n_vis", names_prefix="fbscan_", values_to="fbscan") 

# Keep imputed values from observed visits
liver2 <- dat %>% 
  select(id, visdate, n_vis) %>% 
  left_join(liver, by=c("id", "n_vis"))


# Export data -------------------------------------------------------------

write_csv(liver2, file="../data/alive_liver_impute.csv")

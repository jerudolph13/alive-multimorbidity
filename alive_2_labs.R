...
###################################################################################################################
#
# Purpose: Explore ALIVE lab data
#
...
# This applies to diabetes, lung disease, renal disease, liver fibrosis, anemia
...
# Summarize by year
lab_annual <- make_annual(dat, sym("hba1c"), 
                               sym("fev_fvc"), sym("maxfev"), sym("maxfvc"), 
                               sym("gfr"), sym("uprt_crt"), sym("ucreat"), sym("uprotn"),
                               sym("fbscan"), sym("fbscgrp"), sym("fbscge12_3"),
                               sym("bpdias"), sym("bpsys"),
                               sym("bmimedh"),
                               sym("hemo"))
...
jpeg(file="../figures/lab_values/hba1c_by_yr.jpg", height=5, width=5, units="in", res=300)  
...
# Distribution across all visits
summary(select(dat, fev_fvc, maxfev, maxfvc))

# Distribution by calendar year
summ_fevfvc <- lab_annual %>% 
  filter(yr>2006 & yr<2020) %>% 
  filter(fev_fvc <= 1) %>% # One record (ID 50497) had improbable value in 2016
  group_by(yr) %>% 
...
jpeg(file="../figures/lab_values/lung_by_yr.jpg", height=5, width=5, units="in", res=300)
...
# Distribution across all visits
summary(select(dat, gfr, uprt_crt, ucreat, uprotn))
...
jpeg(file="../figures/lab_values/gfr_by_yr.jpg", height=5, width=5, units="in", res=300)
...
jpeg(file="../figures/lab_values/uprot_by_yr.jpg", height=5, width=5, units="in", res=300)
...
# Distribution across all visits
summary(select(dat, fbscan, fbscgrp, fbscge12_3))
...
jpeg(file="../figures/lab_values/fbscan_by_yr.jpg", height=5, width=5, units="in", res=300)
...
# Distribution across all visits
summary(select(dat, bpdias, bpsys))
...
jpeg(file="../figures/lab_values/bpdias_by_yr.jpg", height=5, width=5, units="in", res=300)
...
jpeg(file="../figures/lab_values/bpsys_by_yr.jpg", height=5, width=5, units="in", res=300)
...
jpeg(file="../figures/lab_values/bmi_by_yr.jpg", height=5, width=5, units="in", res=300)
...
# Distribution across all visits
summary(select(dat, hemo))

# Distribution by calendar year
summ_hemo <- lab_annual %>% 
  filter(yr>2013 & yr<2020) %>% 
  group_by(yr) %>% 
  summarize(p5 = quantile(hemo, probs=0.05, na.rm=T),
            p25 = quantile(hemo, probs=0.25, na.rm=T),
            mean = mean(hemo, na.rm=T),
            med = median(hemo, na.rm=T),
            p75 = quantile(hemo, probs=0.75, na.rm=T),
            p95 = quantile(hemo, probs=0.95, na.rm=T))

jpeg(file="../figures/lab_values/hemo_by_yr.jpg", height=5, width=5, units="in", res=300)
ggplot(data=summ_hemo, aes(x=yr)) + thm1 +
  labs(x="\nCalendar Year", y="Hemoglobin Concentration (g/dL)") +
  geom_ribbon(aes(ymin=p5, ymax=p95), alpha=0.3) +
  geom_ribbon(aes(ymin=p25, ymax=p75), alpha=0.5) +
  geom_line(aes(y=med), linetype="solid") +
  scale_x_continuous(expand=c(0,0), breaks=seq(2014, 2019, 1)) +

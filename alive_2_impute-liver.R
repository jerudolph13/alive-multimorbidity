...
#
# Purpose: Run random forest missing data model for liver stiffness
#
...
# Condition data
comorbid_annual <- make_annual(comorbid, sym("fbscan")) %>% 
  mutate(fbscan=log(fbscan))
...
liver <- wide.impute2 %>% 
  select(id, vars_select(names(wide), starts_with("fbscan"))) %>% 
  pivot_longer(cols=starts_with("fbscan"), names_to="yr", names_prefix="fbscan_", values_to="fbscan") %>% 
  mutate(fbscan = exp(fbscan))

# Export data -------------------------------------------------------------

write_csv(liver, file="../data/alive_liver_impute.csv")

...
# Include participants who have a 2nd visit within 365 days of first visit
  # Start clock at 2nd observed visit
dat2 <- dat %>% 
  group_by(id) %>% 
  ...
  filter(duplicated(id) | (!duplicated(id) & t_between<=365))
...
last <- filter(dat4, !duplicated(id, fromLast=T))
...
# Set up date ranges
  ...
  # Visits before then are removed
...
test <- dat6 %>% mutate(state = case_when(
                                  censor==1 ~ 0,
                                  dead==1 ~ 3,
                                  n_cond<2 ~ 1,
                                  T ~ 2))

res <- tidy(survfit(Surv(days0, days, as.factor(state)) ~ 1, data=test, id=id)) %>% 
